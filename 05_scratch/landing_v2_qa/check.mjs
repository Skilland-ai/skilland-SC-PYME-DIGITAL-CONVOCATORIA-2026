...
const root =
  "file:///home/reboot/Escritorio/Skilland.ai/skilland-SC-PYME-DIGITAL-CONVOCATORIA-2026/04_outputs/00_landing_runs/2026-05-21_africantech_mauritania_knowledge_transfer/03_html/landing_web_v2/index.html";
...
const chrome = spawn(
  ...
  [
    ...
    "--user-data-dir=/tmp/landing-v2-chrome-qa",
    "about:blank",
  ],
...
async function openPage(url) {
  ...
  if (!response.ok) {
    throw new Error(`Could not open target: ${response.status}`);
  }
...
async function runCase(name, width, height, url, clickSpanish = false) {
  ...
  const result = await client.send("Runtime.evaluate", {
    ...
    expression: `(() => {
      ...
      const active = document.querySelector("[data-toggle-lang].is-active")?.getAttribute("data-toggle-lang");
      const visibleSpanish = Array.from(document.querySelectorAll("[data-lang='es']")).filter((node) => getComputedStyle(node).display !== "none").length;
      const visibleEnglish = Array.from(document.querySelectorAll("[data-lang='en']")).filter((node) => getComputedStyle(node).display !== "none").length;
      return {
